...
RED.view = function() {
    ...
    $("#chart").droppable({
            ...
            drop: function( event, ui ) {
                ...
                nn.outputs = nn._def.outputs.length;
                nn.changed = true;
    ...
    function nodeMouseUp(d) {
        ...
        portMouseUp(d, d._def.inputs.length > 0 ? 1 : 0, 0);
    }
    ...
    function redraw() {
        ...
        if (mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function(d,i) {
                ...
                var max_input_label =  d._def.inputs.length==0? "" : d._def.inputs.map(function (o) {return o.name;}).reduce(function (a, b) { return a.length > b.length ? a : b; });
                var max_output_label = d._def.outputs.length==0? "" : d._def.outputs.map(function (o) {return o.name;}).reduce(function (a, b) { return a.length > b.length ? a : b; });
                var portLabels = max_input_label + max_output_label;
                
                var l = d._def.label;
                l = (typeof l === "function" ? l.call(d) : l)||"";
                d.w = Math.max(node_width,calculateTextWidth(l, "node_label")+calculateTextWidth(portLabels, "port_label"));
                d.h = Math.max(node_height,Math.max(d._def.outputs.length||0, d._def.inputs.length||0) * 15);
                ...
                if (d._def.inputs.length > 0) {
                    text.attr("x", calculateTextWidth(max_input_label, "port_label"));
                }
            ...
            node.each(function(d,i) {
                if (d.dirty) {
                    ...
                    thisNode.selectAll(".node_label_right").attr('x', function(d){return d.w-23-(d._def.outputs.length>0?5:0);});
                    thisNode.selectAll(".node_icon_right").attr("x",function(d){return d.w-16-(d._def.outputs.length>0?5:0);});
                    
                    var numInputs = d._def.inputs.length;
                    d.inputs = numInputs;
                    ...
                    var g = d._inports.enter().append("g")
                    ...
                    g.append("rect").attr("class","port port_input").attr("rx",3).attr("ry",3).attr("x",-5).attr("width",10).attr("height",10)
                    .attr("y",function(d,i){return (y+13*i)-5;})
                    g.append("text").text(function(dp, i) {return d._def.inputs[i].name;}).attr("class", "port_input_label port_label")
                    .attr("y",function(d,i){return (y+13*i) + 3;}).attr("x", 7)
                        
                    d._inports.exit().remove();
                    if (d._inports) {
                        var numInputs = d._def.inputs.length || 1;
                        var y = (d.h/2)-((numInputs-1)/2)*13;
                        d._inports.each(function(d,i) {
                                var port = d3.select(this);
                                port.attr("y",(y+13*i)-5).attr("x",-5);
                        });
                    }

                    var numOutputs = d._def.outputs.length;
                    var y = (d.h/2)-((numOutputs-1)/2)*13;
                    d.ports = d.ports || d._def.outputs;
                    d._ports = thisNode.selectAll(".port_output").data(d.ports);
                    g = d._ports.enter().append("g").attr("class", "port_output")
                        ...
                        .on("mouseout",function(d,i) { var port = d3.select(this); port.classed("port_hovered",false);});
                    g.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                    .attr("x",d.w - 5).attr("y", function(d,i){return (y+13*i)-5;})
                    g.append("text").text(function(dp, i) {return d._def.outputs[i].name;}).attr("class", "port_output_label port_label").attr("text-anchor", "end")
                    .attr("x",d.w - 6).attr("y", function(d,i){return (y+13*i) + 2;})
                    
                    d._ports.exit().remove();
                    if (d._ports) {
                        var numOutputs = d._def.outputs.length || 1;
                        var y = (d.h/2)-((numOutputs-1)/2)*13;
                        var x = d.w;
                        d._ports.each(function(d,j) {
                                var port = d3.select(this);
                                port.selectAll(".port").attr("y",function(d,i){return (y+13*j)-5}).attr("x",x - 5);
                                port.selectAll(".port_output_label").attr("y",function(d,i){return (y+13*j)+2}).attr("x",x - 6);
                        });
                    }
        ...
        link.attr("d",function(d){
                ...
                var dy = (d.target.y+ty)-(d.source.y+sy);
                ...
                return "M "+(d.source.x+d.source.w/2)+" "+(d.source.y+sy)+
                    " C "+(d.source.x+d.source.w/2+scale*node_width)+" "+(d.source.y+sy+scaleY*node_height)+" "+
                    (d.target.x-d.target.w/2-scale*node_width)+" "+(d.target.y+ty-scaleY*node_height)+" "+
                    (d.target.x-d.target.w/2)+" "+(d.target.y+ty);
        })
