...
RED.nodes = function() {
    ...
    function convertNode(n) {
        ...
        if (n._def.category != "config") {
            ...
            node.z = n.z;
            node.wires = {};
            for(var i=0;i<n._def.outputs.length;i++) {
                node.wires[n._def.outputs[i].name] = [];
            }
            var wires = links.filter(function(d){return d.source === n;});
            for (var i in wires) {
                var w = wires[i];
                node.wires[w.source._def.outputs[w.sourcePort].name].push({"node": w.target.id, "port": w.target._def.inputs[w.targetPort].name});
            }
        }
        return node;
    }
    ...
    function indexOf(myArray, property, searchTerm) {
                ...
                for (var w1 in n.wires) {
                    ...
                    for (var w2 in wires) {
                        if (wires[w2].node in node_map) {
                            ...
                            var link = {source:n,sourcePort:indexOf(n._def.outputs, "name", w1),target:node_map[wires[w2].node], 
                              targetPort:indexOf(node_map[wires[w2].node]._def.inputs, "name", wires[w2].port)};
                            addLink(link);
                            new_links.push(link);
                        }
                    }
